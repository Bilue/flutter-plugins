...
public class SwiftHealthPlugin: NSObject, FlutterPlugin {
    ...
    let SWIMMING_STROKES = "SWIMMING_STROKES"
            ...
            case let (samplesWorkout as [HKWorkout]) as Any:
                ...
                let dictionaries = samplesWorkout.map { sample -> NSDictionary in
                    return [
                        ...
                        "source_id": sample.sourceRevision.source.bundleIdentifier,
                        "source_name": sample.sourceRevision.source.name,
                        "total_distance": sample.totalDistance?.doubleValue(for: self.unitLookUp(key: self.DISTANCE_WALKING_RUNNING)) ?? 0,
                        "total_energy_burned": sample.totalEnergyBurned?.doubleValue(for: self.unitLookUp(key: self.ACTIVE_ENERGY_BURNED)) ?? 0,
                        "workout_activity_type" : sample.workoutActivityType.name,
                        "total_flights_climbed" : sample.totalFlightsClimbed?.doubleValue(for: self.unitLookUp(key: self.FLIGHTS_CLIMBED)) ?? 0,
                        "total_swimming_stroke_count" : sample.totalSwimmingStrokeCount?.doubleValue(for: self.unitLookUp(key: self.SWIMMING_STROKES)) ?? 0,
                    ]
                }
    ...
    func initializeTypes() {
        ...
        unitDict[SWIMMING_STROKES] = HKUnit.count()
        ...
        if #available(iOS 11.0, *) {
            ...
            dataTypesDict[SWIMMING_STROKES] = HKSampleType.quantityType(forIdentifier: .swimmingStrokeCount)!
